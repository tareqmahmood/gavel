...
from job import Job
from job_table import JobTable, SmallJobTable, LargeJobTable, JobScaleTable
...
def generate_job(throughputs, reference_worker_type='v100', rng=None,
    ...
    job_template = rng.choice(JobScaleTable[scale_factor])
            ...
            for row in reader:
                if i == 0:
                    ...
                    for (header_elem, row_elem) in zip(header[1:], row[1:]):
                        ...
                        per_instance_type_spot_prices[header_elem][zone].append(
                            (date, row_elem))
                i += 1
    return per_instance_type_spot_prices
...
def get_policy(policy_name, solver=None, seed=None,
    ...
    if policy_name.startswith('allox'):
        ...
        policy = \
            finish_time_fairness.FinishTimeFairnessPolicyWithPerf(
                solver=solver)
